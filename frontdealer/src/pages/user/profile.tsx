...
import { getImageUrl } from '../../utils/getBaseURL'; // pastikan path sesuai project
...
useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setLoading(true);
        const response = await fetch('/api/dealer/dealer/profile', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    const fetchProfilePicture = async () => {
      try {
        const response = await fetch('/api/dealer/dealer/profile/picture/get', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
        ...
        if (response.ok) {
  const data = await response.json();
  setImageUrl(data.imageUrl ? getImageUrl(data.imageUrl) : "");
}
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/dealer/dealer/profile/picture/upload', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      // ✅ Refresh gambar jika ada URL dikembalikan
      if (result.data?.Image) {
  const newImageUrl = getImageUrl(`/uploads/${result.data.Image}`);
  setImageUrl(newImageUrl);
}
 else {
        console.warn("Tidak ada image URL dikembalikan dari backend");
      }
  ...
  const handleDeletePicture = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/dealer/dealer/profile/picture/delete', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    <><Navbar /><div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
          ...
          <div className="mb-8 text-center">
              <div className="w-32 h-32 mx-auto rounded-full overflow-hidden border-4 border-gray-200">
                  {imageUrl ? (
                      <img src={imageUrl} alt="Profil" className="w-full h-full object-contain" />
                  ) : (
