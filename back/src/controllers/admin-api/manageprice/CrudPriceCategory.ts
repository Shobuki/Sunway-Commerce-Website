import { Request, Response, NextFunction } from "express";
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

class ProductCategory {
  validatePriceCategoryInput = (input: any) => {
    if (!input.Name || input.Name.length > 40) return 'Nama kategori maksimal 40 karakter';
    return null;
  };
  createPriceCategory = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const { Name } = req.body;
    const errorMsg = this.validatePriceCategoryInput(req.body);
    if (errorMsg) {
      res.status(400).json({ message: errorMsg });
      return;
    }
    try {
      if (!Name) {
        res.status(400).json({ message: "Name is required." });
        return;
      }

      const newPriceCategory = await prisma.priceCategory.create({
        data: {
          Name,
        },
      });

      res.status(201).json({ message: "PriceCategory created successfully.", data: newPriceCategory });
    } catch (error) {
      console.error("Error creating PriceCategory:", error);
      next(error);
    }
  };

  getAllPriceCategories = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const priceCategories = await prisma.priceCategory.findMany({
        include: {
          Price: true, // Include related prices
          Dealer: true, // Include related dealers
        },
      });

      res.status(200).json({ data: priceCategories });
    } catch (error) {
      console.error("Error fetching PriceCategories:", error);
      next(error);
    }
  };

  updatePriceCategory = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const { Id } = req.params;
    const { Name } = req.body;

    try {
      if (!Name) {
        res.status(400).json({ message: "Name is required." });
        return;
      }

      const updatedPriceCategory = await prisma.priceCategory.update({
        where: { Id: parseInt(Id, 10) },
        data: { Name },
      });

      res.status(200).json({ message: "PriceCategory updated successfully.", data: updatedPriceCategory });
    } catch (error) {
      console.error("Error updating PriceCategory:", error);
      next(error);
    }
  };

  deletePriceCategory = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const { Id } = req.params;

    try {
      // Check if PriceCategory is linked to any Price or Dealer
      const relatedPrices = await prisma.price.findMany({
        where: { PriceCategoryId: parseInt(Id, 10) },
      });

      const relatedDealers = await prisma.dealer.findMany({
        where: { PriceCategoryId: parseInt(Id, 10) },
      });

      if (relatedPrices.length > 0 || relatedDealers.length > 0) {
        res.status(400).json({
          message: "Cannot delete PriceCategory with related Prices or Dealers.",
        });
        return;
      }

      await prisma.priceCategory.delete({
        where: { Id: parseInt(Id, 10) },
      });

      res.status(200).json({ message: "PriceCategory deleted successfully." });
    } catch (error) {
      console.error("Error deleting PriceCategory:", error);
      next(error);
    }
  };
}

const productCategoryController = new ProductCategory();

export const createPriceCategory = productCategoryController.createPriceCategory;
export const getAllPriceCategories = productCategoryController.getAllPriceCategories;
export const updatePriceCategory = productCategoryController.updatePriceCategory;
export const deletePriceCategory = productCategoryController.deletePriceCategory;
